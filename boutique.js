...
const boutique = require("./BoutiqueS");
...
boutiquerouter.post("/addboutique", async (req, res) => {
  try {
    ...
    res.send({ boutique: result, msg: "sucefuly aded" });
  } catch (error) {
    res.send({ msg: "fail" });
    console.log(error);
  }
...
boutiquerouter.get("/allboutique", async (req, res) => {
  try {
    ...
    res.send({ boutique: result, msg: "all boutiques " });
  } catch (error) {
    res.send({ msg: "fail" });
    console.log(error);
  }
